...
In the \Cref{tab:questoesgarousi} are the questions used in the decision to carry out the review of the gray literature and their answers. In addition, the objectives defined for carrying out the review were:
...
\input{artifacts/research-questions-garousi.tex}
...
\input{artifacts/defined-rq.tex}
...
The keyword ``\acs{SIGAA}'' was removed after the first search because it is a tool used by many public universities \cite{das2013sistema}, which cluttered the results with essentially the same record, potentially hiding other solutions. The defined strings are presented in \Cref{tab:gl-strings}.

\input{artifacts/search-strings.tex}
...
\input{artifacts/exclusion-criteria.tex}
...
\input{artifacts/quality-criteria.tex}
...
\input{artifacts/search-results.tex}
...
Once all the data had been extracted and tabulated, it was possible to classify the tools using the previously defined quality criteria. 0 (zero) is the minimum and 5 (five) is the maximum score for a tool. The final results obtained are displayed in \Cref{fig:gl-quality-criteria-results}.
